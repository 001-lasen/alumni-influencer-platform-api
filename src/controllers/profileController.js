...
async function createQualifications(req, res) {
    ...
    try {
        ...
        await Promise.all([
            ...
            licences.length > 0 ? models.licences.bulkCreate(licences.map(l => ({ ...l, userId, createdBy: userId, updatedBy: userId }))) : Promise.resolve(),
            courses.length > 0 ? models.professionalCourses.bulkCreate(courses.map(c => ({ ...c, userId, createdBy: userId, updatedBy: userId }))) : Promise.resolve(),
        ]);
